...
class tokensRepository {
    ...
    async updateRefreshSession(userID, refreshToken) {
        try {
            ...
            const refreshSession = await db.query(`SELECT * FROM tokens WHERE user_id = ?`, userID)
            ...
            const updateRefreshToken =
                await db.query(`UPDATE tokens SET refresh_token = ? WHERE user_id = ?`, [refreshToken, userID])
    ...
    async deteleRefreshSession(refreshToken) {
        try {
            const token = await db.query(`DELETE FROM tokens WHERE refresh_token = ?`,
                [refreshToken]
            )
            return token
        } catch (error) {
