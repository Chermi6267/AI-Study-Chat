...
router.post('/registration',
    [
        ...
        check('username', 'Слишком длинное имя пользователя').isLength({ max: 255 }),
        check('password', 'Пароль не может быть пустым').notEmpty(),
        check('password', 'Пароль не может быть короче 8 символов').isLength({ min: 8 }),
        check('password', 'Обязательна заглавная, строчная буквы, цифра и спецсимвол').matches(/(?=.*[0-9])(?=.*[!@#$%^&_*])(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z!@#$%^&*]{6,}/),
        ...
        check('email', 'Слишком длинный адрес электронной почты').isLength({ max: 255 })
    ],
